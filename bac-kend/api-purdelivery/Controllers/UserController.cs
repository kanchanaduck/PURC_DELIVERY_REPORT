...
namespace api_purdelivery.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public UserController(DataBaseContext context, IRepository repository, IConfiguration config)
        {
            _config = config;
            _context = context;
        ...
        public async Task<ActionResult<T_User>> Login(Login Input)
        {
            ...
            if(username!=null){
                ...
                if (user)
                {
                    ...
                    var query = await _context.T_User.Where(e=>e.username==Input.username).FirstOrDefaultAsync();
            ...
            if(username==null){
                try	
                {
                    ...
                    if(res.data.Count>0){  
                        ...
                        try
                        {   
                            return CreatedAtAction("get_users", new { username = register.username }, register);
                        }
        ...
        public async Task<ActionResult<T_User>> EditRole(string username, Register register)
        {
            ...
            user.roles.Clear();
            // T_Role_Claim roles = new T_Role_Claim();
            foreach (var item in register.roles)
            {
        ...
        {
            var customer = _context.T_User
            .Include(c => c.roles)
            .Where(c => c.roles.Any(i => i.role=="Admin"))
            .Count();
